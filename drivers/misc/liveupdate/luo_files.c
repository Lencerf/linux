 ...
 * &liveupdate_filesystem.reboot, &liveupdate_filesystem.finish, etc.)
 ...
 *
 * 3. State Persistence (FDT): During the LUO prepare/reboot phases, the
...
/*
 * Preserved file descriptor array, protected via luo_state_rwsem, indexed by
 * token
 */
static DEFINE_XARRAY(luo_files_xa);
static LIST_HEAD(luo_filesystems_list);

static bool luo_files_xa_recreated;
 ...
 *                 the implementation of prepare, reboot, cancel, and finish
...
static int luo_files_recreate_luo_files_xa(void)
{
	...
	fdt_for_each_subnode(file_node_offset, luo_fdt_in, parent_node_offset) {
		...
		ret = xa_err(xa_store(&luo_files_xa, token, luo_file,
				      GFP_KERNEL));
		if (ret < 0) {
...
int luo_files_fdt_setup(void *fdt)
{
	...
	pr_err("luo_files_fdt_setup 2, xa=%llx\n", (u64)&luo_files_xa);
...
/**
 * luo_do_files_reboot_calls - Calls reboot callbacks and updates FDT
 * if all calls succeed. Handles cancellation on failure.
 *
 * Phase 1: Calls 'reboot' for all files and stores results temporarily.
 * If any 'reboot' fails, calls 'cancel' on previously called files.
 * and returns the error.
 * Phase 2: If all 'reboot' calls succeeded, writes the stored data to the FDT.
 ...
 * @token: Return argument for the token value.
...
int luo_register_file(u64 *tokenp, struct file *file)
{
	...
	ret = down_write_killable(&luo_state_rwsem);
	if (ret < 0) {
		pr_warn("Register file: Interrupted [%d]\n", ret);
		kfree(luo_file);
		return ret;
	}

	if (!liveupdate_state_normal()) {
		pr_warn("File can be registered only in normal state\n");
		ret = -EBUSY;
		kfree(luo_file);
		goto exit_unlock;
	}
 ...
 * global list (protected by luo_state_rwsem) and removes it. This function
 * *only* removes the entry from the list; it does *not* free the memory
 * allocated for the &struct luo_file itself. The caller is responsible
 * for freeing the structure after this function returns successfully.
 *
...
int luo_unregister_file(u64 token)
{
	...
	if (!liveupdate_state_normal()) {
		pr_warn("File can be unregistered only in normal state\n");
		up_write(&luo_state_rwsem);
		return -EBUSY;
	}

	luo_file = xa_erase(&luo_files_xa, token);
	if (luo_file) {
		luo_file->fs->refcnt--;
		kfree(luo_file);
	} else {
 ...
 *
 * Searches the global list (protected by luo_state_rwsem) for a
 * &struct luo_file matching the @token. Uses a read lock, allowing
...
int luo_retrieve_file(u64 token, struct file **file)
{
	...
	ret = luo_files_recreate_luo_files_xa();
	if (ret)
		return ret;
	...
	luo_file = xa_load(&luo_files_xa, token);
...
int liveupdate_unregister_filesystem(struct liveupdate_filesystem *fs)
{
	...
	if (fs->refcnt) {
		ret = -EBUSY;
		pr_err("Can't unregister filesystem, it has registered with LUO file descriptors\n");
		goto exit_unlock;
	}
