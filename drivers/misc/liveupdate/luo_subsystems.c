...
/* Registered subsystem list, protected via luo_state_rwsem */
...
/**
 * luo_do_subsystems_reboot_calls - Calls reboot callbacks and updates FDT
 * if all reboots succeed. Handles cancellation on failure.
 *
 * Phase 1: Calls 'reboot' for all subsystems and stores results temporarily.
 * If any 'reboot' fails, calls 'cancel' on previously called subsystems
 * and returns the error.
...
int luo_do_subsystems_reboot_calls(void)
{
	...
	list_for_each_entry(subsystem, &luo_subsystems_list, list) {
		...
		ret = subsystem->reboot(subsystem->arg,
					&subsystem->private_data);
		if (ret < 0) {
			pr_err("Subsystem '%s' reboot callback failed [%d]\n",
			       subsystem->name, ret);
			__luo_do_subsystems_cancel_calls(subsystem);
 ...
 *
 * Registers a subsystem handler that provides callbacks for different phases
 * of the live update cycle. Registration is typically done during the
 * subsystem's module init or core initialization.
 *
 * Can only be called when LUO is in the NORMAL state.
 * The provided name (@h->name) must be unique among registered subsystems.
 *
 * Return: 0 on success, negative error code otherwise.
 * -EINVAL if h or h->name is NULL or h->name is empty.
 * -EEXIST if a subsystem with the same name is already registered.
 ...
 *
 * Can only be called when LUO is in the NORMAL state. Acquires the
 * luo_state_rwsem for writing.
 *
 * Return: 0 on success, negative error code otherwise.
 * -EINVAL if h is NULL.
