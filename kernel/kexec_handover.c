 ...
 */
 ...
 /**
  * kho_preserve_folio - preserve a folio accross KHO.
 ...
 int kho_preserve_folio(struct folio *folio)
 {
	 ...
	 return __kho_preserve(&kho_mem_track, pfn, order);
 }
 ...
 {
	 unsigned int order = ilog2(size >> PAGE_SHIFT);
	 unsigned long end_pfn = PHYS_PFN(phys + size);
	 unsigned long pfn, failed_pfn;
	 int err;

	 for (pfn = PHYS_PFN(phys); pfn < end_pfn;
