...
int kho_get_node(const struct kho_in_node *parent, const char* name, struct kho_in_node *child) {
	...
	offset = fdt_subnode_offset(kho_in.handover_fdt, parent_offset, name);
	if (offset < 0)
		return -ENOENT;
...
int kho_get_nodes(const struct kho_in_node *parent, int (*func)(const char *, const struct kho_in_node *, void *), void *data) {
	...
	fdt_for_each_subnode(child.offset, kho_in.handover_fdt, parent_offset) {
		if (child.offset < 0)
			return -EINVAL;

		name = fdt_get_name(kho_in.handover_fdt, child.offset, NULL);
...
const void *kho_get_prop(const struct kho_in_node *node, const char *key, u32 *size) {
	...
	return fdt_getprop(kho_in.handover_fdt, offset, key, size);
}
...
int kho_node_check_compatible(const struct kho_in_node *node, const char *compatible) {
	...
	return fdt_node_check_compatible(kho_in.handover_fdt, node->offset, compatible);
}
...
int kho_copy_fdt(struct kimage *image)
{
	...
	fdt = kimage_map_segment(image, image->kho.fdt->mem, PAGE_ALIGN(kho_out.dt_max));
	if (!fdt) {
...
static __init int kho_in_debugfs_init(const void *fdt)
{
	...
	kho_in.fdt.size = fdt_totalsize(fdt);
	kho_in.fdt.data = (void *)fdt;
	file = debugfs_create_blob("fdt", 0400, kho_in.dir, &kho_in.fdt);
	if (IS_ERR(file)) {
		err = PTR_ERR(file);
		goto err_rmdir;
	}
...
static __init int kho_init(void)
{
	...
	if (kho_in.handover_fdt) {
		err = kho_in_debugfs_init(kho_in.handover_fdt);
		/*
			 ...
			 u64 scratch_len)
{
	...
	fdt_size = fdt_totalsize(handover_dt);
	kho_in.handover_fdt = phys_to_virt(handover_dt_phys);

	early_memunmap(handover_dt, sizeof(struct fdt_header));

	/* Reserve the DT so we can still access it in late boot */
	memblock_reserve(handover_dt_phys, fdt_size);
