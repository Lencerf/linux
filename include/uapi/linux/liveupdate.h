 ...
 * @LIVEUPDATE_STATE_FROZEN:         The final reboot notification
 ...
 * some finalization might occur during %LIVEUPDATE_IOCTL_EVENT_REBOOT.
 ...
 * the kernel restores the state (saved during the PREPARE/REBOOT phases)
 ...
 *                               %LIVEUPDATE_IOCTL_EVENT_REBOOT. Note that some
...
/**
 * LIVEUPDATE_IOCTL_EVENT_REBOOT - Notify subsystems of imminent reboot
 * transition.
 *
 * Argument: None.
 *
 ...
 * via kexec). This action triggers the internal %LIVEUPDATE_REBOOT kernel
 ...
 * ioctls typically occurs in response to the %LIVEUPDATE_REBOOT event.
 *
 * Importantly, this ioctl command itself does **not** initiate the system
 * reboot. It serves purely as a notification. The caller is expected to invoke
 * the actual ``reboot()`` system call shortly after this ioctl returns
 * successfully, or alternatively, this event might be called internally from
 * the `reboot()` syscall pathway itself just before the transition.
 *
 ...
 */
#define LIVEUPDATE_IOCTL_EVENT_REBOOT					\
