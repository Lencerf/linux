...
static int reserve_mem_kho_finalize(struct kho_serialization *ser)
{
	...
	err |= kho_preserve_folio(ser, kho_fdt);
	err |= kho_record_folio(ser, MEMBLOCK_KHO_FDT, kho_fdt, KHO_FOLIO_FDT);
...
static void *__init reserve_mem_kho_retrieve_fdt(void)
{
	...
	err = kho_retrieve_folio(MEMBLOCK_KHO_FDT, KHO_FOLIO_FDT, &fdt_phys);
	if (err) {
		if (err != -ENOENT)
