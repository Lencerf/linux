...
static int reserve_mem_kho_finalize(struct kho_serialization *ser)
{
	...
	for (i = 0; i < reserved_mem_count; i++) {
		...
		err |= kho_preserve_phys(ser, NULL, map->start, map->size);
	}

	err |= kho_preserve_folio(ser, MEMBLOCK_KHO_FDT, kho_fdt);

	fdt = page_to_virt(folio_page(kho_fdt, 0));
	kho_register_preserved_mem(ser, MEMBLOCK_KHO_FDT, fdt, PAGE_SIZE);
...
static void __init prepare_kho_fdt(void)
{
	...
	kho_fdt = folio_alloc(GFP_KERNEL, 0);
	if (!kho_fdt) {
...
static void *__init reserve_mem_kho_retrieve_fdt(void)
{
	...
	err = kho_retrieve_folio(MEMBLOCK_KHO_FDT, &fdt_phys);
	if (err) {
		if (err != -ENOENT)
