...
static struct liveupdate_filesystem shmem_luo_fs_ops = {
	.prepare = shmem_luo_prepare,
	.cancel = shmem_luo_cancel,
	.finish = shmem_luo_finish,
	.retrieve = shmem_luo_retrieve,
	.compatible = shmem_luo_compatible,
	.can_preserve = shmem_luo_can_preserve,
};
...
static int __init shmem_luo_init(void)
{
	...
	err = liveupdate_register_filesystem(&shmem_luo_fs_ops);
	if (err)
